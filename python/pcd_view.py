...
HEADER_IDENTIFIER = 0x55AA5AA5  # 包头识别符
...
point_cloud_item.setData(pos=points[:, :3], color=(1, 0, 0, 1), size=3)  # 更新点云大小为1
...
def update_point_cloud(data):
    ...
    # 解析数据包
    packet_frame_number = struct.unpack('H', data[4:6])[0]  # 数据包中的帧号
    ...
    for i in range(0, PIXEL_COUNT * 4, 4):
        ...
        vertical_angle = -VERTICAL_ANGLE_RANGE + (i // 4) * VERTICAL_ANGLE_STEP  # 垂直俯仰角
